...
def transform(rows):
    ...
    for row in rows:
        ...
        # Если скобок нет или строка пустая - пропускаем
        if not match:
            continue
        ...
        flst = []
        # 2. Безопасное преобразование в float
        for item in raw_items:
            item = item.strip()
            if not item:
                continue
            try:
                # Это отфильтрует символы  и другие ошибки
                flst.append(float(item))
            except ValueError:
                continue

        if len(flst) < SUBCARRIER * 2:
                continue
        flst_clean = []
