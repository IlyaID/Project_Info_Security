...
class chat_manager:
    def __init__(self,IP,PORT, TARGET_PORT):
        self.IP = IP
        self.PORT = PORT
    ...
    def send_init(self):
        ...
        self.send_data.connect((self.IP,self.TARGET_PORT))
    
    def recv_init(self):
        self.stop = False
        self.recv = socket(AF_INET,SOCK_DGRAM)
        self.recv.setsockopt(SOL_SOCKET, SO_REUSEADDR, 1)
        self.recv.bind(('',self.PORT))
