...
/* --- State --- */
static bool s_stack_inited = false; // nvs, netif, loop
static bool s_wifi_inited  = false; // esp_wifi_init called
static bool s_wifi_started = false; // esp_wifi_start called

/* --- Current Config --- */
static int s_channel = 11;
static wifi_second_chan_t s_second = WIFI_SECOND_CHAN_BELOW;
static wifi_bandwidth_t s_bw = WIFI_BW_HT40;
...
{
    /* 1. Set MAC (требует остановки или до старта) */
    if (s_mac_set) {
...
static int cmd_radio_init(int argc, char **argv)
{
    ...
    if (nerrors) {
        arg_print_errors(stderr, radio_args.end, argv[0]);
        return 1;
    }
...
static int cmd_radio_tx_power(int argc, char **argv)
{
    ...
    if (nerrors != 0) {
        arg_print_errors(stderr, tx_power_args.end, argv[0]);
        return 1;
    }

    if (!radio_is_inited()) {
        printf("Error: Radio not initialized. Run 'radio_init' first.\n");
        return 1;
    }

    if (tx_power_args.power->count > 0) {
        int dbm = tx_power_args.power->ival[0];
        // ESP32 units: 0.25dBm. (e.g. 80 = 20dBm)
        int8_t power_unit = (int8_t)(dbm * 4); 
        esp_err_t err = esp_wifi_set_max_tx_power(power_unit);
        if (err != ESP_OK) {
    ...
    int8_t cur_power = 0;
    esp_err_t err = esp_wifi_get_max_tx_power(&cur_power);
    if (err == ESP_OK) {
...
static int cmd_radio_scan(int argc, char **argv)
{
    ...
    /* Auto-init if needed */
    if (!radio_is_inited()) {
        printf("Radio not initialized. Initializing defaults...\n");
        radio_init_csi_defaults();
    }

    wifi_scan_config_t scan_config = {
        .ssid = NULL, .bssid = NULL, .channel = 0, .show_hidden = true,
        .scan_type = (scan_args.passive->count > 0) ? WIFI_SCAN_TYPE_PASSIVE : WIFI_SCAN_TYPE_ACTIVE,
        .scan_time.active.min = 100, .scan_time.active.max = 300, .scan_time.passive = 300
    };
    ...
    printf("\nFound %d APs:\n", ap_count);
    printf("----------------------------------------------------------------------------------\n");
    printf("| %-32s | %-17s | %3s | %3s | %4s |\n", "SSID", "BSSID", "Ch", "Sec", "RSSI");
    printf("----------------------------------------------------------------------------------\n");
    ...
    for (int i = 0; i < ap_count; i++) {
        ...
        char safe_ssid[33];
        strncpy(safe_ssid, (char*)ap_list[i].ssid, 32);
        safe_ssid[32] = '\0';
        for(int j=0; safe_ssid[j]; j++) { 
            if(safe_ssid[j] < 32 || safe_ssid[j] > 126) safe_ssid[j] = '.'; 
        }

        printf("| %-32s | %s | %3d | %3d | %4d |\n", 
               safe_ssid, bssid_str, ap_list[i].primary, ap_list[i].second, ap_list[i].rssi);
    }
    printf("----------------------------------------------------------------------------------\n");
